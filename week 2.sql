...
select count(distinct driver_id) CNT from participated a,accident1 b where a.report_num=b.report_num and b.accident_date like '2008%';
...
select p.name,c.damage_amt from person p,participated c where p.driver_id=c.driver_id and c.damage_amt>(select avg(damage_amt) from participated);
select * from cars order by year asc;
select count(report_num) from cars c,participated p where c.reg_num=p.reg_num and c.model='Lancer';
select count(distinct driver_id) CNT from participated a,accident1 b where a.report_num=b.report_num and b.accident_date like '2008%';
delete from participated where damage_amt<(select t.damount from   (select avg(damage_amt) as damount from participated)t);
